                        ...
                        <c:if test="${requestScope.count>0}">
                            ...
                            <c:forEach var="i" begin="${startPage+1}" end="${endPage-1}">
                                <c:choose>
                                    ...
                                    <c:when test="${pageNum!=i}"><a
                                            href="/sales/search?pageNum=${i}&ctg=${ctg}&keyword=${keyword}&order3=${order}">${i}</a></c:when>
                                </c:choose>
                            </c:forEach>

                            <!-- 현재 페이지 묶음의 마지막 페이지보다 페이지가 더 존재하면 -->
                            <c:if test="${endPage<=pageCount}">
                                <a href="/sales/search?pageNum=${startPage+11}"><i
                ...
                if (count > 0) {
                    ...
                    for (let j = startPage + 1; j <= endPage - 1; j++) {
                        if (pageNum == j) {
                            ...
                            message += "<a href='/sales/sales?pageNum=" + j + "&ctg=" + ctg + "&keyword=" + keyword + "&order3=" + order + "'>" + j + "</a>"
                        }
                    }
